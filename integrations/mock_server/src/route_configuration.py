...
class RouteResponse:
    ...
    body: Any  # Dict for inline JSON, or str for $ref content (raw file bytes)
...
def resolve_ref(body: Dict[str, Any], base_path: Path) -> str:
    """
    ...
    Returns:
        str: The raw file content as a string (preserves exact bytes for hash verification).
    ...
    """
    ref_path = base_path / body["$ref"]
    log.debug("Resolving $ref: %s -> %s", body["$ref"], ref_path)
    ...
    try:
        ...
        for route_config in config["routes"]:
            ...
            if isinstance(body, dict) and "$ref" in body:
                # $ref: load raw file content as string (preserves bytes for hash verification)
